...
import { validateHandle, resolveHandle } from '../../../identity.js';
  ...
  return {
    ...
    handler: async ({ input }) => {
      ...
      if (!did) {
        ...
        // check that we can resolve it (also so it gets cached)
      }
      const resolvedDID = await resolveHandle(handle, pds);
      if (resolvedDID !== did) {
        throw new InvalidRequestError('Handle did not resolve to DID', 'UnsupportedDomain');
      }
